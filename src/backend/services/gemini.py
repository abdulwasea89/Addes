"""Google Gemini LLM service for the AI pipeline (Agno-backed).

Three operations, each forcing **structured Pydantic output** via an Agno
:class:`agno.agent.Agent` wrapped around :class:`agno.models.google.Gemini`:

* :func:`clean` — turn scraped content into a brand record (brand_name,
  tagline, key_benefits, tone, colors, industry).
* :func:`outline` — turn a brand record into ad copy (headline, body, CTA,
  keywords).
* :func:`craft_prompt` — turn an outline plus brand into a minimalist
  image-generation prompt string.

Why Agno
--------
Previously this module talked to ``google-genai`` directly. Moving the LLM
call behind an :class:`Agent` gives us a single, uniform structured-output
contract that is shared with :mod:`backend.services.groq` (so the router's
dispatch table doesn't need to know which provider enforces JSON server-side
vs client-side), plus room to grow into tools/memory/streaming later without
rewriting the call sites.

Model choice
------------
Default model is ``gemini-2.5-flash`` — fast, cheap, and supports structured
output. Callers can override per-call via the ``model`` argument.

Public API
----------
The function signatures, return dict shape, and :class:`GeminiError` contract
are **unchanged** so the router and tests don't need to know we swapped
implementations.
"""
...
class GeminiError(RuntimeError):
    """Raised when the Gemini call fails or returns unparseable output."""


# ── Internal output schemas ───────────────────────────────────────────
# These live here as standalone Pydantic models so the Agent can hand them to
# Gemini as a strict JSON schema. The router-facing schemas
# (``CleanResponse`` etc.) keep their own ``model_used`` field, which the
# service fills in after the model returns.
...
def _make_agent(
    ...
    """Build a fresh Agno :class:`Agent` bound to Gemini with structured output.

    Per-call instantiation is intentional: ``Agent`` is cheap to construct
    (no network until the first ``arun``), and a fresh agent keeps each call
    stateless so per-call ``model`` overrides Just Work.
    """
...
async def _run_agent(
    ...
    Any provider error (auth, rate limit, network) is collapsed to a single
    :class:`GeminiError` so the router's existing 502 handler can treat all
    ...
    if not isinstance(content, schema):
        raise GeminiError(
            f"Gemini returned unexpected payload type for {schema.__name__}: "
...
__all__ = ["DEFAULT_MODEL", "GeminiError", "clean", "craft_prompt", "outline"]
